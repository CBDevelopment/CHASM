...
from datasets.folder_utils import extract_date, get_file_by_date
...
class FITSDataset():
    ...
    def select_best_hough_circle(self, circles, image):
        ...
        for circle in circles[0, :]:
            ...
            distance_to_center = np.sqrt((circle_center[0] - image_center[0])**2 +
        ...
        best_circle = Circle(int(best_circle[0]), int(
            best_circle[1]), int(best_circle[2]))
        return best_circle
    ...
    def crop_masks_to_circle(self, mask_file, circle: Circle):
            ...
            for mask in mask_file['info']:
                if mask['mask_region'] is None:
                    ...
                    blank_mask = np.zeros((2 * circle.radius, 2 * circle.radius), dtype=np.uint8)
                    cropped_masks.append(blank_mask)
                else:
                    segmentation_mask = mask['mask_region']['segmentation']
                    cropped_mask = segmentation_mask[circle.center_y - circle.radius:circle.center_y + circle.radius,
                                                circle.center_x - circle.radius:circle.center_x + circle.radius]
                    cropped_masks.append(cropped_mask)
            
            return cropped_masks

    def _to_fits(self, image_filename: str, chasm: CHASMDataset, solar_r=959.78, resolution=4096):
        ...
        solar_radius = solar_r  # arcseconds, this is variable based on the oribit of the SDO
        ...
        cropped_masks = cv2.resize(cropped_masks, (int(sdo_solar_radius_pixels) * 2, int(sdo_solar_radius_pixels) * 2), interpolation=cv2.INTER_NEAREST)

        blank_image = np.zeros((resolution, resolution), dtype=np.uint8)
        blank_image_centerx, blank_image_centery = blank_image.shape[1]//2, blank_image.shape[0]//2
        masks_centerx, masks_centery = cropped_masks.shape[1]//2, cropped_masks.shape[0]//2
        ...
        # Save as FITS file
        output_filename = f"{os.path.splitext(os.path.basename(image_filename))[0]}.fits"
        output_filepath = os.path.join(self.root, output_filename)
        sunpy_map.save(output_filepath, overwrite=True)
    ...
    def build(self, drawings: DrawingsDataset, chasm: CHASMDataset):
        ...
        def crop_masks(drawings: DrawingsDataset, chasm: CHASMDataset, year=None):
            ...
            for image_file in tqdm(image_files, desc="Cropping Image Files in Folder"):
                if image_file.endswith(".jpg"): # Ignores README.md selection_times.csv, etc.
                    try:
        ...
        if not os.path.exists(self.root):
            ...
            print(f"Root {self.root} already exists. Cannot build to existing directory. Exiting")
