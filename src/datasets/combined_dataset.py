...
class CombinedDataset():
    ...
    def build_prediction_dataset(
        ...
        chasm_ds = (
            ...
            else CHASMDataset(root=chasm, fetch_online=fetch_online, needs_aia=False) if chasm else None
        )

        fits_ds = (
            fits if isinstance(fits, FITSDataset)
            else FITSDataset(root=fits) if fits else None
        )
