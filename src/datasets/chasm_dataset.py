...
class CHASMDataset(AutoDownloadDataset):
    ...
    def set_mode_train(self):
        self.mode = "train"
        self.file_paths = [
            item for item in self.all_file_paths
    ...
    def get_aia_days(self):
        j = json.load(open(os.path.join(self.root, "missing_wavelength_dates.json")))["MISSING_WAVELENGTHS"]
        return j
    
    def get_pre_timeshift_days(self):
        j = json.load(open(os.path.join(self.root, "missing_wavelength_dates.json")))["PRE_TIMESHIFT"]
        return j
...
class CHASM1407(CHASMDataset, Dataset):
    ...
    def __init__(self, root="download_data/chasm_as_npz", fetch_online=True, needs_aia=True):
        super().__init__(root, fetch_online)
        self.root = root
        self.needs_aia = needs_aia
        # TODO get aia days properly from an aia path
        if needs_aia:
            self.aia_days = self.get_aia_days() # TODO fix this is broken right now! (have it link to actual AIA folder)
        else:
            self.aia_days = []
        self.prune_to_1407()
