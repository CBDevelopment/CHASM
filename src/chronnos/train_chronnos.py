...
parser.add_argument(
    "--base_path", type=str, help="the path to store the training results."
)
parser.add_argument(
    "--data_path",
    type=str,
...
parser.add_argument(
    ...
    "--channels",
    nargs="+",
    help="set to use a subset of wavelenghts.",
    required=False,
    default=None,
)
...
if args.convert.lower() == "true":
    ...
    grouped_files = get_intersecting_files(
        args.data_path,
        dirs=["94", "131", "171", "193", "211", "304", "335", "6173", "prep_masks"],
        extensions=[".fits"] * 8 + [".fits.gz"],
    )
    convertMaps(
        grouped_files[:-1], os.path.join(args.converted_path, "map"), resolutions
    )
    convertMasks(
        grouped_files[-1], os.path.join(args.converted_path, "mask"), resolutions
    )

channels = (
    None
    if args.channels is None
    else [list(sdo_norms_dict.keys()).index(int(wl)) for wl in args.channels]
)
