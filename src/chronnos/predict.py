...
parser = argparse.ArgumentParser(
    description="Predict CHRONNOS masks from SDO FITS files"
)
parser.add_argument(
    "--data_path",
    type=str,
...
parser.add_argument(
    "--model_path", type=str, help="path to the pretrained CHRONNOS model"
)
parser.add_argument("--evaluation_path", type=str, help="path to the output files")
parser.add_argument(
    "--no_reproject",
    action="store_false",
    help="reproject coronal hole maps to the original AIA map.",
)
...
parser.add_argument(
    "--n_workers",
    type=int,
    help="number of parallel threads for converting data",
    default=8,
)
...
ch_generator = chronnos_detector.ipredict(
    map_paths, reproject=reproject, num_workers=args.n_workers
)
for ch_map, aia_map_path in tqdm(
    zip(ch_generator, map_paths[3]), total=len(map_paths[3])
):
    # save fits
    ...
    if plot_samples:
        ...
        plt.savefig(
            os.path.join(
                evaluation_path, os.path.basename(aia_map_path).replace(".fits", ".jpg")
            ),
            dpi=100,
        )
        plt.close()
