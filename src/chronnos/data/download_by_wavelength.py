...
class SingleWavelengthFetcher:
    ...
    async def process_download(self, header, segment, t):
        wavelength = str(
            header.get("WAVELNTH", "6173")
        )  # Default to HMI wavelength if not found
    ...
    async def fetchDates(self, dates):
        connector = aiohttp.TCPConnector(
            limit=self.num_worker_threads, force_close=True
        )
        ...
        try:
            for date in dates:
                ...
                file_path = os.path.join(
                    self.ds_path, self.wavelength, f"{formatted_time}.fits"
                )
                ...
                try:
                    ...
                    logging.error(
                        f"Error fetching {date.isoformat()}: {traceback.format_exc()}"
                    )
    ...
    async def fetchData(self, time):
        ...
        if self.wavelength == "6173":  # HMI magnetogram
            ...
            header, segment = self.drms_client.query(
                ds, key=",".join(keys), seg="magnetogram"
            )

            if len(header) != 1 or np.any(header.QUALITY != 0):
                logging.error(
                    f"HMI data not valid for {time.isoformat()}. Using fallback."
                )
                await self.fetchDataFallback(time)
                return

            await self.download_queue.put(
                (header.iloc[0].to_dict(), segment.iloc[0].magnetogram, time)
            )
            ...
            keys = self.drms_client.keys(ds)
            header, segment = self.drms_client.query(
                ds, key=",".join(keys), seg="image"
            )

            if len(header) != 1 or np.any(header.QUALITY != 0):
                logging.error(
                    f"EUV data not valid for {time.isoformat()}. Using fallback."
                )
                await self.fetchDataFallback(time)
                return

            await self.download_queue.put(
                (header.iloc[0].to_dict(), segment.iloc[0].image, time)
            )
    ...
    async def fetchDataFallback(self, time):
        ...
        logging.info(
            f"Found valid data with time difference: {header_df.iloc[0]['date_diff']}"
        )
        ...
        await self.download_queue.put(
            (header.to_dict(), getattr(segment, seg_type), time)
        )


async def download_multiple_wavelengths(
    base_path, wavelengths, dates, hmi_series="hmi.M_720s", n_workers=4, resolution=None
):
    """
    ...
    for wavelength in wavelengths:
        ...
        logging.info(
            f"Completed wavelength {wavelength}Å in {hours:02d}:{minutes:02d}:{seconds:02d}"
        )
...
async def parse_analysis_file(
    file_path: str,
) -> Tuple[Dict[str, List[datetime]], Dict[str, List[Tuple[datetime, Set[str]]]]]:
    """
    ...
    for section in year_sections:
        ...
        missing_dates_section = re.search(
            r"SWPC dates with no wavelength data.*?\n(.*?)\n\n", section, re.DOTALL
        )
        ...
        missing_wavelengths_section = re.search(
            r"Days with missing wavelengths:.*?\n(.*?)(?:\n\n|$)", section, re.DOTALL
        )
        if missing_wavelengths_section:
            for line in missing_wavelengths_section.group(1).strip().split("\n"):
                if "SWPC Drawing Timestamp:" in line:
                    ...
                    timestamp_match = re.search(r"(\d{8})_(\d{4})", line)
                    wavelengths_match = re.search(
                        r"Missing wavelengths: \[(.*?)\]", line
                    )
                    ...
                    if timestamp_match and wavelengths_match:
                        ...
                        wavelengths = {
                            w.strip() for w in wavelengths_match.group(1).split(",")
                        }
...
async def download_missing_data(analysis_file: str, base_path: str):
    ...
    """
    completely_missing_dates, partially_missing_dates = await parse_analysis_file(
        analysis_file
    )
    ...
    for year, date_wavelength_pairs in partially_missing_dates.items():
        ...
        for wavelength, dates in wavelength_to_dates.items():
            ...
            await download_multiple_wavelengths(
                base_path=base_path, wavelengths=[wavelength], dates=dates
            )
    ...
    for year, dates in completely_missing_dates.items():
        ...
        if dates:
            ...
            await download_multiple_wavelengths(
                base_path=base_path, wavelengths=wavelengths, dates=dates
            )
...
async def main():
    ...
    parser.add_argument(
        "--base_path", type=str, required=True, help="Base path for downloading files"
    )
