...
)  # TODO figure out if this only works for bool, that could be problem
...
def pad_to_square(arr, pad_value=0):
    ...
    padded_arr = np.pad(
        arr, ((0, pad_rows), (0, pad_cols)), mode="constant", constant_values=pad_value
    )
    return padded_arr


def getMapData(
    file, resolution, correction_table=None, remove_off_disk=False, calibrate=True
):
    ...
    if isinstance(s_map, HMIMap):  # truncate boundary for HMI images
        ...
        # TODO figure out why this isn't working. Where is s_map coming from and why doesn't it have exptime
...
# def buildFITS(prediction: np.array, reference: str, reproject=True, threshold=None):
...
#     :param threshold: float value for optional thresholding. If None, keep raw values.
...
#         data_to_map = output
#     else:
#         data_to_map = prediction

#     # Apply threshold only if explicitly set
#     if threshold is not None:
#         data_to_map = (data_to_map >= threshold).astype(np.float32)  # keep as float if you want 0/1
#     else:
#         data_to_map = data_to_map.astype(np.float32)  # keep raw continuous values

#     pred_map = Map(data_to_map, ref_map.wcs)
...
# TODO REMOVE THIS OLD VERSION AFTER CONFIRMING THRESHOLDED VERSION IS OVER
def buildFITS(prediction: np.array, reference: str, reproject=True, threshold=False):
    """Returns a '~sunpy.map.Map' from the mask based on the WCS information of the reference file.
    ...
    if reproject:
        output, footprint = reproject_interp(prep_map, ref_map.wcs, ref_map.data.shape)
        if threshold == False:
            print("DEBUG BUILD FITS 1st branch trigger")
            pred_map = Map(output, ref_map.wcs)
        else:
            print("DEBUG BUILD FITS 2nd branch trigger")
            pred_map = Map((output >= threshold), ref_map.wcs)
    else:
        if threshold == False:
            print("DEBUG BUILD FITS 3rd branch trigger")
            pred_map = Map(prediction, prep_map.wcs)
        else:
            print("DEBUG BUILD FITS 4th branch trigger")
            pred_map = Map((prediction >= threshold), prep_map.wcs)

    print("BUILD MAP PRED MAP ", pred_map.data)  # TODO gets turned iinto nan, fix this!
    pred_map.meta["rsun_obs"] = ref_map.meta["rsun_obs"]
    pred_map.meta["instrume"] = "CHRONNOS"
    pred_map.meta["detector"] = "CHRONNOS"
    pred_map.meta["CDELT1"] *= 60 * 60
...
def convertMapInference(
    grouped_files, converted_path, resolution, n_workers=8, replace=False
):
    """Convert FITS file to full resolution for inference.
    ...
    :return: None
    """
    os.makedirs(converted_path, exist_ok=False)

    correction_table = get_local_correction_table()
    converter = _MapConverter(converted_path, [resolution], correction_table, replace)
    # async conversion
    with Pool(n_workers) as p:
        [
            ...
            for _ in tqdm(
                p.imap_unordered(
                    converter.convert, np.array(grouped_files).transpose()
                ),
                total=len(grouped_files[0]),
            )
        ]
...
def convertMaps(grouped_files, converted_path, resolutions, n_workers=8, replace=False):
    ...
    [
        os.makedirs(os.path.join(converted_path, "%d" % res), exist_ok=True)
...
class _MapConverter:
    ...
    def convert(self, c_files):
        ...
        try:
            ...
            maps_data = [
                getMapData(c_file, self.max_res, self.correction_table)
            ...
            for resolution in self.resolutions:
                ...
                ):  # If only one resolution, no need to make specific folder for it. Makes inference smoother
                    path = os.path.join(
                        self.converted_path,
                        os.path.basename(c_files[0]).replace(".fits", ".npy"),
                    )
            ...
            print(f"Exception {e} raised on {c_files}")
...
def convertMasks(files, converted_path, resolutions, n_workers=8, replace=False):
    ...
    with Pool(n_workers) as p:
        [
            None
            for _ in tqdm(p.imap_unordered(converter.convert, files), total=len(files))
        ]
...
def get_intersecting_files(path, dirs, months=None, years=None, extensions=None):
    ...
    basenames = [
        [
            os.path.basename(path).replace(ext, "")
            for path in glob.glob(
                os.path.join(path, str(d), "**", "*%s" % ext), recursive=True
            )
        ]
    ...
    return [
        [os.path.join(path, str(dir), b + ext) for b in basenames]
        for dir, ext in zip(dirs, extensions)
    ]
