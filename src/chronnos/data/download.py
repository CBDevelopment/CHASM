...
class DataSetFetcher:
    def __init__(
        ...
        self.session = None
        self.semaphore = None
        self.map_executor = ThreadPoolExecutor(
            max_workers=4
                    ...
                    [
                        os.path.exists(
                            os.path.join(
                                self.ds_path,
                                dir,
                                date.isoformat("T", timespec="seconds").replace(":", "")
                                + ".fits",
                            )
                        )
    ...
    async def fetchDataFallback(self, time):
        ...
        logging.info(
            f"Found valid magnetogram data with time difference: {header_df.iloc[0]['date_diff']}"
        )
        ...
        logging.info(
            f"Successfully retrieved all EUV data for {len(futures)} wavelengths"
        )

        await self.download_queue.put(
            (header_hmi.to_dict(), segment_hmi.magnetogram, time)
        )
        ...
        for h, s in zip(header_euv, segment_euv):
            ...
            logging.debug(
                f"Added EUV data for wavelength {h.get('WAVELNTH', 'unknown')}Å to download queue"
            )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download AIA and HMI files for CHRONNNOS image segmentation."
    )
    parser.add_argument(
        "--path", type=str, default=os.path.join(Path.home(), "chronnos")
    )
