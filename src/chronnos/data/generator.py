    ...
    def __getitem__(self, index):
        ...
        x = np.array(
            [
                ...
                for file in file_cube
            ]
        )
        x = x * 2 - 1  # scale to [-1, 1]
        print("ITEM GOTTEN IN CHRONNOS FITS DATASET ", x)
...
class CombinedCHDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if random.random() < self.flip_prob:
            x = np.flip(x, axis=2)
            y = np.flip(y, axis=2)
        return np.array(x.data.tolist(), dtype=np.float32), np.array(
            y.data.tolist(), dtype=np.float32
        )
...
# TODO implement this train test split into the dataset paradigm you have
def getDataSet(ds_path, resolution, train_months=None):
    ...
    mask_files = sorted(
        glob.glob(
            os.path.join(os.path.join(ds_path, "mask", "%d" % resolution), "*.npy")
        )
    )
    map_files = sorted(
        glob.glob(
            os.path.join(os.path.join(ds_path, "map", "%d" % resolution), "*.npy")
        )
    )

    basename_mask = [os.path.basename(f) for f in mask_files]
    basename_map = [os.path.basename(f) for f in map_files]
    assert np.all(np.array(basename_mask) == np.array(basename_map)), (
        "Filenames of masks and maps need to match!"
    )
