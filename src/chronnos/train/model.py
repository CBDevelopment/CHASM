...
from chronnos.train.callback import (
    PlotCallback,
    ValidationCallback,
)
...
class Trainer:
    def __init__(
        self, base_path, data_path, device=None, channels=None, **training_args
    ):
        ...
        self.model = CHRONNOS(
            training_args["n_dims"],
            training_args["n_convs"],
            training_args["image_channels"],
            1,
            dropout=0.2,
        )
        self.opt = torch.optim.Adam(
            self.model.parameters(), lr=1e-3, betas=(0.0, 0.99), weight_decay=1e-8
        )

        self.criterion = BCELoss(reduction="none")
        self.lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(
            optimizer=self.opt, gamma=0.5
        )
    ...
    def train(self):
        ...
        for i in range(1, len(self.training_args["n_dims"])):
            ...
            self.opt.add_param_group(
                {
                    "params": list(self.model.down_block.parameters())
    ...
    def _trainStep(self, resolution_id):
        ...
        save_model_path = os.path.join(
            self.base_path, "ch_%04d_%s_model.pt" % (resolution_id, fade_flag)
        )
        ...
        train_files_map, train_files_mask, valid_files_map, valid_files_mask = (
            getDataSet(self.ds_path, resolution_id)
        )
        train_ds = CombinedCHDataset(
            train_files_map, train_files_mask, channel=self.channels
        )
        ...
        valid_loader_map = DataLoader(
            valid_map_ds, batch_size=batch_size, shuffle=False, num_workers=4
        )
        valid_loader_mask = DataLoader(
            valid_mask_ds, batch_size=batch_size, shuffle=False, num_workers=4
        )

        pred_callback = PlotCallback(
            data=[
                (valid_map_ds[i], valid_mask_ds[i])
                for i in range(0, len(valid_map_ds), len(valid_map_ds) // 10)
            ],
            prefix="%04d_%s" % (resolution_id, fade_flag),
            path=self.base_path,
            model=self.model,
            cmaps=np.array(sdo_cmaps)[self.channels] if self.channels else None,
        )
        self.model.eval()
        with torch.no_grad():
            pred_callback.call(-1, 0.0)
        epoch_alphas = np.linspace(0, 1, epochs, endpoint=False)
        for epoch, epoch_alpha in enumerate(epoch_alphas):
            alphas = np.linspace(
                epoch_alpha, epoch_alpha + np.diff(epoch_alphas)[0], len(train_loader)
            )
            alpha = epoch_alpha
            self.model.train()
            for (x, y), alpha in zip(train_loader, alphas):
                ...
                total_count = y_pred.numel()
                nan_percentage = 100.0 * nan_count / total_count

                print(f"NaNs: {nan_count} / {total_count} ({nan_percentage:.4f}%)")
            ...
            iou, acc = self.validation_callback.call(
                self.model, valid_loader_map, valid_loader_mask, alpha
            )
            logging.info(
                "EPOCH %03d/%03d [IOU %.03f] [ACC %.03f]"
                % (epoch + 1, epochs, iou, acc)
            )
        torch.save(
            {
                "m": self.model.state_dict(),
                "o": self.opt.state_dict(),
                "history": self.validation_callback.history,
            },
            save_model_path,
        )
...
class CHRONNOS(nn.Module):
    def __init__(
        self, n_dims: List, n_convs: List, image_channels, n_classes, dropout=0.2
    ):
        super().__init__()
        ...
        self.core = CoreModel(
            n_dims[self.step], n_convs[self.step], dropout=self.dropout
        )
    ...
    def createFadeIn(self):
        assert self.fade == False, (
            "Model is already in fade in mode. Use createFixed first."
        )
        self.step += 1
        self.fade = True
        self.from_image_fade = FromImageModel(
            self.image_channels, self.n_dims[self.step]
        )
        self.down_block = DownBlock(
            self.n_dims[self.step],
            self.n_dims[self.step - 1],
            self.n_convs[self.step],
            self.dropout,
        )
    ...
    def createFixed(self):
        assert self.fade is True, (
            "Model is already in fixed mode. Use createFadeIn first."
        )
        self.fade = False
    ...
    def forwardFadeIn(self, image, alpha):
        assert self.fade == True, (
            "Model currently in fade in mode. Use forward instead."
        )
    ...
    def forward(self, image):
        assert self.fade is False, (
            "Model currently in fixed mode. Use forwardFadeIn instead."
        )
...
class SCAN(nn.Module):
    def __init__(self, channel_models, n_classes, n_dims, n_convs, dropout=0.2):
        super().__init__()
        self.fade = False

        self.channels = len(channel_models)
        self.merge_layers = [
            nn.Conv2d(n_dims[i] * self.channels, n_dims[i], 1, bias=False)
            for i in range(len(n_dims))
        ]

        self.models = channel_models
        self.up_blocks = [
            UpBlock(n_dims[i - 1], n_dims[i], n_convs[i - 1], dropout=dropout)
            for i in range(1, len(n_dims))
        ]
        self.to_img = ToMaskModel(n_dims[-1], n_classes)

        self.module_list = nn.ModuleList(
            [*self.models, *self.up_blocks, self.to_img, *self.merge_layers]
        )
        self.train_modules = nn.ModuleList(
            [*self.up_blocks, self.to_img, *self.merge_layers]
        )
    ...
    def forwardCombined(self, image):
        ...
        channel_masks = [
            m.forward(image[:, i : i + 1]) for i, m in enumerate(self.models)
        ]
...
class DownBlock(nn.Module):
    def __init__(self, dim, out_dim, n_convs, dropout=0.2):
        super().__init__()
        assert n_convs >= 1, (
            "Invalid configuration, requires at least 1 convolution block"
        )
        self.convs = nn.Sequential(
            *[Conv2dBlock(dim, dim, dropout=dropout) for _ in range(n_convs)]
        )
...
class UpBlock(nn.Module):
    def __init__(self, dim, out_dim, n_convs, n_skip=1, dropout=0.2):
        ...
        self.convs = nn.Sequential(
            *[
                Conv2dBlock(
                    out_dim * (1 + n_skip) if i == 0 else out_dim,
                    out_dim,
                    dropout=dropout,
                )
