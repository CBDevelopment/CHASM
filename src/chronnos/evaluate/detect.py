...
class CHRONNOSDetector:
    def __init__(self, model_path=None, model_name="chronnos_v1_0.pt", device=None):
        ...
        """
        assert (model_path is not None) or (model_name is not None), (
            "Either model_path or model_name need to be specified."
        )
    ...
    def predict_dir(self, data_path, dirs=None, **kwargs):
        ...
        """
        dirs = (
            ["94", "131", "171", "193", "211", "304", "335", "6173"]
            if dirs is None
            else dirs
        )
    ...
    def predict(
        self, map_paths, reproject=False, num_workers=4, calibrate=True, threshold=False
    ):
        ...
        map_loader = DataLoader(
            map_ds, batch_size=2, shuffle=False, num_workers=num_workers
        )
        ...
        res_maps = []
        for prediction, ref in tqdm(
            zip(predictions, map_paths[3]),
            desc="converting to Map",
            total=len(predictions),
        ):
            s_map = buildFITS(
                prediction[0], ref, reproject=reproject, threshold=threshold
            )
    ...
    def ipredict(
        ...
        map_loader = DataLoader(
            map_ds, batch_size=1, shuffle=False, num_workers=num_workers
        )
        ...
        with torch.no_grad():
            for batch, ref in tqdm(zip(map_loader, map_paths[3]), desc="Predicting"):
                ...
                s_map = buildFITS(
                    pred, ref, reproject=reproject, threshold=threshold
                )  # TODO make this not BINARY
...
def load_model_path(model_name):
    ...
    if not os.path.exists(model_path):
        request.urlretrieve(
            "http://kanzelhohe.uni-graz.at/iti/" + model_name, filename=model_path
        )
    return model_path
