...
def find_closest_file(directory, target_datetime):
    ...
    for file_path in directory_path.iterdir():
        if file_path.suffix == ".fits":
            ...
            timestamp_str = file_path.stem  # Get the part before '.fits'
...
def main(date, time, wavelength):
    ...
    try:
        im = solar_map.plot(
            axes=ax, cmap=ct.aia_color_table(int(wavelength) * u.angstrom)
        )
    except:
    ...
    plt.figtext(
        0.5, 0.01, f"Observation Date: {observation_date}", ha="center", fontsize=10
    )
    plt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--time", type=str, help="Time of the solar image", default="T000000"
    )
    parser.add_argument(
        "--wavelength", type=str, help="Wavelength of the solar image", default="193"
    )
