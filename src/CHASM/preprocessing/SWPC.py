    ...
    def get_synoptic_map(date: str, time: str, save_path: str):
        ...
        if response.status_code == 200:
            ...
            if len(image_candidates) == 0:
                ...
                best_candidate_time = int(best_candidate['href'].split('_')[-1].split('.')[0])
                for candidate in image_candidates:
                    candidate_time = int(candidate['href'].split('_')[-1].split('.')[0])
                    if abs(candidate_time - int(f"{hours}{minutes}")) < abs(best_candidate_time - int(f"{hours}{minutes}")):
                ...
                if image_response.status_code == 200:
                    with open(os.path.join(save_path, best_candidate['href']), 'wb') as file:
