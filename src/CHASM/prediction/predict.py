...
from chronnos.evaluate.detect import (
    CHRONNOSDetector,
)
import torch
...
def build_prediction_dataset(
    ...
    chasm_ds = (
        ...
        else CHASMDataset(root=chasm, fetch_online=fetch_online, needs_aia=True)
        if chasm
        else None
    )
    ...
    fits_ds = (
        ...
        if isinstance(
            fits, FITSDataset
        )  # TODO in documentation and everything make distinction between this and internal FITSDataet from original repo
        else FITSDataset(root=fits)
        if fits
        else None  # TODO check what difference calibrate makes
    )
...
def build_prediction_from_base(basefolder: str, model_path: str):
    ...
    return build_prediction_dataset(
        drawings=str(base / "drawings"),
        aia=str(base / "aia"),
        chasm=CHASM1111(root=str(base / "chasm"), fetch_online=True, needs_aia=False),
        fits=str(base / "fits"),
        chronnos=str(base / "chronnos"),
        prediction_path=str(base / "predictions"),
        model_path=model_path,
        device="cuda",
        force_recompute=False,
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Build prediction dataset from a base folder"
    )
    parser.add_argument("model_path", help="Path to the trained model file (.pt)")
    parser.add_argument(
        "basefolder",
        help="Top-level folder containing drawings, aia, chasm, fits, chronnos_maps, predictions",
    )
