...
class ThreadedImageLoader:
    ...
    def get_matching_sam_match(self, next_idx):
        ...
        mask_file = get_file_by_date(self.load_dir.filenames()[next_idx], self.masks_dir.filenames())
        ...
        while not self.should_stop:
            ...
            try:
                with self.loading_lock:
                    ...
                    while (os.path.exists(os.path.join(self.save_dir, "DATA-" + str(self.load_dir.filenames()[next_idx][:-4] + ".npz")))):
                        next_idx += 1
                    self.current_idx = next_idx

                    # Skip README.md
                    while os.path.exists(os.path.join(self.load_dir.root, "README.md")) and \
                            self.load_dir.filenames()[next_idx] == "README.md":
                        next_idx += 1

                    # Load and process image
                    file_name = self.load_dir.filenames()[next_idx]
                    ...
                    scaled_image = cv2.resize(img_array, (scaled_width, scaled_height),
                    ...
                    processed_masks = [masks[arr_str].item() for arr_str in masks
