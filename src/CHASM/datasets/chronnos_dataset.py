...
class _MapConverter:
    def __init__(self, converted_path, resolutions, correction_table, replace):
        ...
        sdo_norms = [
            ImageNormalize(
                vmin=0, vmax=445.5, stretch=AsinhStretch(0.005), clip=True
            ),  # 94
            ImageNormalize(
                vmin=0, vmax=981.3, stretch=AsinhStretch(0.005), clip=True
            ),  # 131
            ImageNormalize(
                vmin=0, vmax=6457.5, stretch=AsinhStretch(0.005), clip=True
            ),  # 171
            ImageNormalize(
                vmin=0, vmax=7757.31, stretch=AsinhStretch(0.005), clip=True
            ),  # 193
            ImageNormalize(
                vmin=0, vmax=6539.8, stretch=AsinhStretch(0.005), clip=True
            ),  # 211
            ImageNormalize(
                vmin=0, vmax=3756, stretch=AsinhStretch(0.005), clip=True
            ),  # 304
            ImageNormalize(
                vmin=0, vmax=915, stretch=AsinhStretch(0.005), clip=True
            ),  # 335
            ImageNormalize(
                vmin=-100, vmax=100, stretch=LinearStretch(), clip=True
            ),  # mag
        ]
        self.sdo_norms_dict = {
    ...
    def convert(self, c_files):
        ...
        try:
                ...
                all(
                    [
                        (
                            ...
                            / str(res)
                            / Path(c_files[0]).name.replace(".fits", ".npy")
                        ).exists()
                        for res in self.resolutions
                    ]
                )
                and self.replace == False
            ):
            ...
            maps_data = [
                self.getMapData(c_file, self.max_res, self.correction_table)
            ...
            for resolution in self.resolutions:
                ...
                block = (
                    maps_data.shape[0] // resolution,
                    maps_data.shape[1] // resolution,
                    1,
                )
    ...
    def pad_to_square(self, arr, pad_value=0):
        ...
        print(
            f"Warning: array is not square ({rows}x{cols}). Padding to {max_size}x{max_size}."
        )
    ...
    def getMapData(
        ...
        s_map = prepMap(
            s_map, resolution
        )  # TODO figure out why this works here, but isn't used in _MaskConvert now to make things work, despite being in OG code
        #
        if isinstance(s_map, HMIMap):  # truncate boundary for HMI images
            ...
            if "exptime" not in s_map.meta and needs_exptime == True:
                logging.error(
                    f"{file} missing EXPTIME but needs_exptime=True. Exiting."
                )
...
def prepMap(s_map: Map, resolution: int, padding_factor: float = 0.1):
    ...
    s_map = s_map.submap(
        SkyCoord(
            -arcs_frame * u.arcsec, -arcs_frame * u.arcsec, frame=s_map.coordinate_frame
        ),
        top_right=SkyCoord(
            arcs_frame * u.arcsec, arcs_frame * u.arcsec, frame=s_map.coordinate_frame
        ),
...
class _MaskConverter:
    ...
    def convert(self, file):
        try:
                ...
                all(
                    [
                        (
                            Path(self.converted_path)
                            / str(res)
                            / Path(file).name.replace(".fits", ".npy")
                        ).exists()
                        for res in self.resolutions
                    ]
                )
            ...
            s_map = fits.getdata(
                file
            )  # NOTE: This was using the "dummy" fits to properly resize. Since we are treating just like an image this isn't needed. TODO could just have the CHRONNOS system we have use npy instead for this reason all the way through.
...
class CHRONNOSDataset:
    ...
    def get_intersecting_files_date(path, dirs, extensions=None):
        ...
        dir_to_files = []
        for directory, ext in tqdm(
            zip(dirs, extensions),
            desc="Zipping Intersecting Files",
            total=min(len(dirs), len(extensions)),
        ):
    ...
    def get_local_correction_table(self):
        ...
        """
        path = Path.home() / "aiapy" / "correction_table.dat"
        if path.exists():
            return get_correction_table(str(path))
        (Path.home() / "aiapy").mkdir(parents=True, exist_ok=True)
        correction_table = get_correction_table(
            "jsoc"
        )  # NOTE: Changed source to JSOC to prevent errors
        astropy.io.ascii.write(correction_table, path)
        return correction_table

    # Convert maps and all the needed helper functions
    def convertMaps(
        self, grouped_files, converted_path, resolutions, n_workers=8, replace=False
    ):
        ...
        [
            (Path(converted_path) / str(res)).mkdir(parents=True, exist_ok=True)
        ...
        converter = _MapConverter(
            converted_path, resolutions, correction_table, replace
        )
        # async conversion
        # TODO could add sync and async option to build that gets passed down
        for g in tqdm(
            np.array(grouped_files),
            desc="Iterating through grouped files",
    ...
    def convertMasks(
        self, files, converted_path, resolutions, n_workers=8, replace=False
    ):
        ...
        with Pool(n_workers) as p:
            [
                None
                for _ in tqdm(
                    p.imap_unordered(converter.convert, files), total=len(files)
                )
            ]
    ...
    def getPredictionInputsDataset(self):
        class NumpyFolderDataset(Dataset):
            def __init__(self, folder_path, transform=None):
                ...
                self.files = [f.name for f in folder.iterdir() if f.suffix == ".npy"]

            def __len__(self):
                return len(self.files)

            def __getitem__(self, idx):
                file_path = Path(self.folder_path) / self.files[idx]
                array = np.load(file_path)
                tensor = torch.from_numpy(array).float()  # convert to float tensor
                return tensor, str(file_path)

        return NumpyFolderDataset(str(Path(self.root) / "inputs" / "map"))

    def get_inputs_path(self):
        return str(Path(self.root) / "inputs")

    def get_map_path(self):
        return str(Path(self.root) / "inputs" / "map")

    def get_mask_path(self):
        return str(Path(self.root) / "inputs" / "mask")

    def get_map_dataset(self, resolution=512):
        map_dir = Path(self.get_map_path()) / str(resolution)
        files = [str(f) for f in map_dir.iterdir()]
        return MapDataset(files)

    def get_mask_dataset(self, resolution=512):
        mask_dir = Path(self.get_mask_path()) / str(resolution)
        files = [str(f) for f in mask_dir.iterdir()]
    ...
    def get_intersecting_files(self):
        ...
        intersecting_dirs = [
            ...
            "335",
            "6173",
            "cropped_centered_masks",
        ]
        chronnos_raw_inputs = str(Path(self.root) / "chronnos_merged_inputs")
        return CHRONNOSDataset.get_intersecting_files_date(
            chronnos_raw_inputs, dirs=intersecting_dirs, extensions=[".fits"] * 8
        )
    ...
    def build(self, aia, fits, verbose=False, inference=True):
        ...
        chronnos_raw_inputs = str(root_path / "chronnos_merged_inputs")
        ...
        CHRONNOSDataset.merge_directories(
            aia_path=aia.root,
            fits_path=fits.root,
            output_path=chronnos_raw_inputs,
            merge_dirs=intersecting_dirs,
        )
        self.grouped_files = CHRONNOSDataset.get_intersecting_files_date(
            chronnos_raw_inputs, dirs=intersecting_dirs, extensions=[".fits"] * 8
        )

        # TODO add nan checked in these functions
        # TODO don't use converted_path and use root instead
        (Path(self.root) / "inputs").mkdir(parents=True, exist_ok=True)
        ...
        self.convertMaps(
            [g[:-1] for g in self.grouped_files[:-1]], map_path, self.config.resolutions
        )
        logging.info("====================== CONVERTING MASKS  ======================")
        self.convertMasks(
            [g[-1] for g in self.grouped_files[:-1]], mask_path, self.config.resolutions
        )

        if not inference:
            CHRONNOSDataset.remove_unmatched_files_recursive(
                map_path, mask_path
            )  # Make sure file names match (prune out non matching from missing aia wavelengths, etc.)
    ...
    def train_models_swpc(self, results_path: str = None):
        ...
        channels = (
            ...
            else [list(sdo_norms_dict.keys()).index(int(wl)) for wl in config.channels]
        )
    ...
    def merge_directories(aia_path, fits_path, output_path, merge_dirs=None):
        ...
        for year in tqdm(years, desc="Merging directories AIA [year]"):
            year_path = (aia_path_obj / year) if year else aia_path_obj
            ...
            if merge_dirs:
                wavelength_folders = [
                    w.name
                    for w in year_path.iterdir()
                    if w.name in merge_dirs and w.is_dir()
                ]
            else:
                wavelength_folders = [w.name for w in year_path.iterdir() if w.is_dir()]

            for wavelength in tqdm(
                wavelength_folders, desc=f"Merging Directories [Wavelength]"
            ):
                wavelength_path = year_path / wavelength
                if not wavelength_path.is_dir():
                    continue

                target_folder = output_path_obj / wavelength
                target_folder.mkdir(parents=True, exist_ok=True)

                for file_path in tqdm(
                    list(wavelength_path.iterdir()),
                    desc="Merging Directories [File Names]",
                ):
        ...
        years = [
            d.name
            for d in fits_path_obj.iterdir()
            if d.is_dir() and any(sub.is_dir() for sub in d.iterdir())
        ]
        if not years:
            years = [None]
    ...
    def remove_unmatched_files_recursive(folder_a, folder_b):
        ...
        for subfolder_a in folder_a_path.iterdir():
            if not subfolder_a.is_dir():
                continue

            subfolder_b = folder_b_path / subfolder_a.name

            # Only process if both subfolders exist
            if not subfolder_b.is_dir():
                print(f"Skipping {subfolder_a}, no matching subfolder in {folder_b}")
                continue

            # Get filenames in both subfolders
            files_a = {f.name for f in subfolder_a.iterdir() if f.is_file()}
            files_b = {f.name for f in subfolder_b.iterdir() if f.is_file()}

            unmatched = files_a - files_b

            for filename in unmatched:
                path_to_remove = subfolder_a / filename
