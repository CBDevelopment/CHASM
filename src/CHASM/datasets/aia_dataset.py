...
class MagResizer:
    def __init__(
        self, root, year, source_folder="6173", target_folder="6173_resampled"
    ):
        self.root = root
        self.year = year
    ...
    def process_single_file(self, fits_file):
        ...
        try:
            ...
            # Save resampled file
            target_path = Path(self.root) / self.year / self.target_folder
            target_path.mkdir(parents=True, exist_ok=True)
            output_path = str(target_path / fits_file)
    ...
    def process_folder(self):
        ...
        fits_dir = Path(self.root) / self.year / self.source_folder
        print("DIR ", fits_dir)
        fits_files = sorted([f.name for f in fits_dir.iterdir() if f.suffix == ".fits"])
        print("RUNNING POOL. ")
