...
# # # # # # # TODO PUT THIS INTO THE DATASET PIPELINE AFTER CHECKING THIS # # # # # # 
...
class MagResizer():
    def __init__(self, root, year, wavelength):
        self.root = root
        self.year = year
    ...
    def process_single_file(self, fits_file):
        ...
        try:
            ...
            mag_map = sunpy.map.Map(f"{self.root}/{self.year}/{self.wavelength}/{fits_file}")
            ...
            with fits.open(f"{self.root}/{self.year}/{self.wavelength}/{fits_file}") as hdul:
            ...
            # Save resampled file
            output_path = f"{self.root}/{self.year}/resampled_mag/resampled_{fits_file}"
    ...
    def view_fits(self, fits_file):
        """View a FITS file."""
        resampled = f"{self.root}/{self.year}/resampled_mag/{fits_file}"
...
if __name__ == '__main__':
    root = r"/home/egsmith/scratch/CoronalHoles/src/polished_pipeline/PREDICTIONS/CHASM1111/aia"
    year = "2024"
    wavelength = 6173

    resizer = MagResizer(root, year, wavelength)
    resizer.main()

    resampled_files = sorted([f for f in os.listdir(f"{root}/{year}/resampled_mag")
                              if f.endswith(".fits")])
    #resizer.view_fits(resampled_files[257])